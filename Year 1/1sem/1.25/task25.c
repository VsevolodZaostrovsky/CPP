...
//функция определяет максимальное расстояние между двумя соседними локальными минимумами целочисленной последовательности
//Ввод - файл "input.txt"
//Вывод - целое число

int main() {
        int r = 0;	//расстояние между двумя соседними минимумами
	int numn = 2;	//номер числа, обрабатываемого в цикле (2, поскольку первые два числа уже обработаны)
	...
	if (fscanf(inp, "%d", &n0) != 1) {  //работа с первым элементом
    	...
    	if (fscanf(inp, "%d", &n1) != 1) {  //работа со вторым элементам
        	printf("В последовательности нет ни одного локального минимума, поскольку она состоит лишь из одного числа\n");
        	return -1;
   	}
